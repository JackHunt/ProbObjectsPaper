...
\begin{figure}[!t]
	\centering
	\subfloat[]{\includegraphics[height=3cm]{graphical_models/pgm1.pdf}\label{pgm1}}%
	\hspace{12mm}%
	\subfloat[]{\includegraphics[height=3cm]{graphical_models/pgm2.pdf}\label{pgm2}}
...
to second-order gradient-based optimisation routines, e.g.\ Gauss-Newton or Levenberg-Marquardt.
...
\begin{equation}
\begin{split}
P(L_{(s, s')} | \Omega, p) \propto P(\Psi_{s}(x) | \Psi_{s'}(\Lambda(x)))P(L_{(s, s')})
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\ln P(\Psi_{s'}(\Lambda(x))) = m\ln\frac{1}{\sqrt{2\pi}\sigma} -\frac{1}{2\sigma^2} \\ 
\sum_{(s, s') \in \mathcal{S}} \Bigg [ \bigg( \Psi_{s}(x) - \Psi_{s'}(\Lambda(x)) \bigg)^2  - \lambda \lVert \mathbf{\mathbf{x}} \rVert_{2} \Bigg ]
\end{split}
\end{equation}
...
the translational and rotational parameters.
