...
of probabilities, pertaining to posteriors over a voxels assignment to being either belonging to the object or not. \STUART{The wording of this needs improvement.} In the proposed system this 
...
first RGB frame. \STUART{Is it true that you're using a bounding box for initialisation rather than a segmentation? If so, why?} At each frame a smaller, image sized volume \STUART{What do you mean by image-sized volume here?} is constructed based on the predictions of the current frame. During the fusion process, this 
