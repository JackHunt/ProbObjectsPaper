To evaluate the performance of our system, we perform quantitative experiments on camera pose estimation accuracy, and qualitatively analyse the obtained reconstructions.
...
This implicitly allows us to track the camera wrt. the object regardless of which of the two is subject to motion.
Finally, qualitative comparisons are drawn between the reconstructions attained by our system and those of the method described in \cite{Ren2013}.
We evaluate our system on multiple sequences depicting objects of different sizes. %; some with the object moving wrt. a fixed camera, others with the sensor in motion.% in the case of qualitative evaluation. 
...
\begin{figure}[!t]
	...
	\end{tabular}
	\caption{
        Quarterly interval snapshots of the Dinosaur Head reconstruction using \textbf{(a)} our method, and \textbf{(b)} the one proposed by Ren et~al.~\cite{Ren2013}.
	}
...
\begin{figure}[!t]
	...
	\end{tabular}
	\caption{
		Closed reconstructions of \textbf{(a)} a Dinosaur Head,
		\textbf{(b)} a Chair, and
		\textbf{(c)} a Rock.
	}
	\label{fig:top_shots}
